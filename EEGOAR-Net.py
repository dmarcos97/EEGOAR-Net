...
from tensorflow.keras.layers import Conv2D, AveragePooling2D, DepthwiseConv2D, Add, MaxPooling2D
...
from tensorflow.signal import fft, hamming_window
from tensorflow import transpose

#HAMMING WINDOW
hm = hamming_window(128)
hm = expand_dims(hm, axis=0)
hm = expand_dims(reshape(repeat(hm, 64, axis=1), [1, 128, 64]), axis=-1)

def custom_mse(y_true,y_pred):
    fft_y_true = transpose(fft(cast(transpose(y_true,
                                                  perm=[3,0,2,1]),complex64)),perm=[1,3,2,0])
...
def EEGOARNET(input_time=1000, fs=128, ncha=64, filters_per_branch=8,
    ...
    # ============================ BLOCK 4: ENCODING  ======================== #
    b4_u1 = Conv2D(filters=int(filters_per_branch*len(scales_samples)*4),
                   kernel_size=(3, 1),
                   kernel_initializer='he_normal',
                   use_bias=False,
                   padding='same')(b3_out)
    ...
    b4_u2 = Conv2D(filters=int(filters_per_branch*len(scales_samples)*4),
                   kernel_size=(3, 1),
                   kernel_initializer='he_normal',
                   use_bias=False,
                   padding='same')(b4_u1)
...
if __name__ == "__main__":
    model = EEGOARNET(filters_per_branch=8)
    model.summary()
